...
@router.post("/json", response_model=DesigningResponse, status_code=201)
async def create_designing_json(
    payload: DesigningCreate,
    background_tasks: BackgroundTasks,
    db: AsyncSession = Depends(get_db),
) -> Designing:
    """Accept application/json (product_images already uploaded and passed directly)."""
